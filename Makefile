...
playground:
	...
	@echo "|                                                                             |"
	@echo "| 🔍 IMPORTANT: Select the 'orchestrator_agent' folder to interact with your agent.          |"
	@echo "==============================================================================="
...
deploy:
	...
	(uv export --no-hashes --no-header --no-dev --no-emit-project --no-annotate > orchestrator_agent/app_utils/.requirements.txt 2>/dev/null || \
	uv export --no-hashes --no-header --no-dev --no-emit-project > orchestrator_agent/app_utils/.requirements.txt) && \
	uv run -m orchestrator_agent.app_utils.deploy \
		--source-packages=./orchestrator_agent \
		--entrypoint-module=orchestrator_agent.agent_engine_app \
		--entrypoint-object=agent_engine \
		--requirements-file=orchestrator_agent/app_utils/.requirements.txt \
...
eval:
	...
	uv run adk eval ./orchestrator_agent $${EVALSET:-tests/eval/evalsets/basic.evalset.json} \
...
register-gemini-enterprise:
	@uvx agent-starter-pack@0.39.2 register-gemini-enterprise